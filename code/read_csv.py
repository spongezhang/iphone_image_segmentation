...
for source in source_image_set:
    ...
    for setting in experiment_setting:
        score_list = load_csv('../multi_region/result/csv_file/' + source + '_dice_region-{}.csv'.format(setting),',')
    ...
    csv_file = csv.writer(open('../final_file/{}_ave.csv'.format(source), 'w'), delimiter=',')
    ...
    for tmp_source in source_image_set:
        tem_list = ['{:.3f}'.format(np.mean(image_dict[tmp_source][setting_idx])) for setting_idx, tmp_setting in enumerate(experiment_setting)]
        csv_file.writerow(tem_list)
        if tmp_source != source:
            for setting_idx, tmp_setting in enumerate(experiment_setting):
                setting_dict[tmp_setting].append(np.mean(image_dict[tmp_source][setting_idx]))

    tem_list = ['{:.3f}'.format(np.mean(np.array(setting_dict[tmp_setting]))) for tmp_setting in experiment_setting]
    csv_file.writerow(tem_list)

    csv_file = csv.writer(open('../final_file/{}_std.csv'.format(source), 'w'), delimiter=',')
    ...
    csv_file.writerow(experiment_setting)
    for tmp_source in source_image_set:
        tem_list = ['{:.3f}'.format(np.std(image_dict[tmp_source][setting_idx])) for setting_idx, tmp_setting in enumerate(experiment_setting)]
        csv_file.writerow(tem_list)
        if tmp_source != source:
            for setting_idx, tmp_setting in enumerate(experiment_setting):
                setting_dict[tmp_setting].append(image_dict[tmp_source][setting_idx])

    tem_list = ['{:.3f}'.format(np.std(np.array(setting_dict[tmp_setting]))) for tmp_setting in experiment_setting]
    csv_file.writerow(tem_list)
...
tem_list = ['{:.3f}'.format(np.mean(np.array(all_setting_dict[tmp_setting]))) for tmp_setting in experiment_setting]
csv_file.writerow(tem_list)
...
tem_list = ['{:.3f}'.format(np.std(np.array(all_setting_dict[tmp_setting]))) for tmp_setting in experiment_setting]
csv_file.writerow(tem_list)
