...
import utils
from one_shot_training_sequence import OneShotTrainingSequence
import train_utils
import U_net
import cv2
import numpy as np
...
training_sequence = OneShotTrainingSequence(training_image_dir, training_mask_dir, da=True, batch_size=4)
...
validation_sequence = OneShotTrainingSequence(test_image_dir, test_mask_dir, da=False, batch_size=1)
...
u_net = model.BuildUnet((image_size[1], image_size[0], input_channels), args.start_filters, args.unet_depth,\
        dropout=False, _nchannel = 1, use_softmax = use_softmax)
...
u_net.fit_generator(
        ...
        steps_per_epoch=len(training_sequence)/args.batch_size,
